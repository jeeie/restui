...
using Photonware.RestUI.WP.Actions;
using Photonware.RestUI.Core;
using Photonware.RestUI.SL8.Utils;
...
namespace Photonware.RestUI.SL8
{
    public partial class App : Application
    {
        ...
        public static UserCaseViewModel SelectedUserCaseViewModel { get; set; }

        public static CustomActionViewModel SelectedCustomActionViewModel { get; set; }
        ...
        public static UserCaseExecution SelectedUserCaseExecution { get; set; }

        public static UserCase SelectedUserCase { get; set; }

        public static ExecutionAction SelectedExecutionActon { get; set; }

        public static Photonware.RestUI.Core.Scripts.IScriptExecutor JavaScriptExecutor
        {
            get { return new Photonware.RestUI.Scripts.JavaScriptExecutor(); }
        }
        /// <summary>
        /// Application 对象的构造函数。
        /// </summary>
        public App()
        {
            ...
            SshTunnelManager.Instance.IsEnabled = false;
            ...
            try
            {
                if (IsolatedStorageSettings.ApplicationSettings.Contains("useSshTunnel"))
                {
                    ...
                    if (IsolatedStorageSettings.ApplicationSettings.TryGetValue<bool>("useSshTunnel", out sshEnabled))
                    {
                        if (sshEnabled)
                        {
                            ...
                            if (IsolatedStorageSettings.ApplicationSettings.Contains("sshHost"))
                            {
                                ...
                                IsolatedStorageSettings.ApplicationSettings.TryGetValue<string>("sshHost", out host);

                            }
                            if (IsolatedStorageSettings.ApplicationSettings.Contains("sshPort"))
                            {

                                IsolatedStorageSettings.ApplicationSettings.TryGetValue<string>("sshPort", out port);
                            }
                            if (IsolatedStorageSettings.ApplicationSettings.Contains("sshUsername"))
                            {
                                IsolatedStorageSettings.ApplicationSettings.TryGetValue<string>("sshUsername", out username);
                            }
                            if (IsolatedStorageSettings.ApplicationSettings.Contains("sshPassword"))
                            {
                                IsolatedStorageSettings.ApplicationSettings.TryGetValue<string>("sshPassword", out password);
                            }
                            SshTunnelManager.Instance.SetParameters(host, Convert.ToInt32(22), username, password);
                            SshTunnelManager.Instance.IsEnabled = true;
                        }
                    }
                }
            }
            ...
            lock (ActionManager.Instance.SyncRoot)
            {
                ...
                ActionManager.Instance.Actions.Add(restAction.Key, restAction);
                 restAction.SshTunnelManager = SshTunnelManager.Instance;
                ...
                jsAction.SshTunnelManager = SshTunnelManager.Instance;

                Photonware.RestUI.WP.Actions.SshAction sshAction = new SshAction();
                ActionManager.Instance.Actions.Add(sshAction.Key, sshAction);
                sshAction.SshTunnelManager = Photonware.RestUI.WP.Utils.SshTunnelManager.Instance;
            ...
            UserCaseManager.Instance.FileUtils = App.FileUtils;
            UserCaseManager.Instance.init();
            System.Diagnostics.Debug.WriteLine(fileUtils.GetHomeFolder());
        }
