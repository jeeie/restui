        ...
        public bool SendRequest(string request)
        {
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    else
                    {
                        if (hwr == null)
                        {
                            ...
                            string host = null;
                            if (SshTunnelManager.Instance.IsEnabled)
                            {
                                UriBuilder ub = new UriBuilder(m.Groups[2].Value);
                                
                                SshTunnelManager.Instance.GetProxyAddress(ub.Uri.Host, ub.Uri.Port, out proxyHost, out proxyPort);
            ...
            if (hwr != null)
            {
                ...
                finally
                {
                    if (SshTunnelManager.Instance.IsEnabled)
                    {
                        SshTunnelManager.Instance.ReleaseSshProxy(proxyHost, proxyPort);
                    }
                }
